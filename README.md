...
### Pruning strategy S1/S2/S3/S4

Pipeline hiện hỗ trợ 4 strategy ở step 2:
...
Với `S1/static`, nếu prune rate là `r`, mỗi layer giữ:
...
- shell ngoài cùng dùng `PRUNE_STRATEGY=S1/S2/S3/S4`
- Python nội bộ dùng `--prune_method static/kneedle/otsu/gmm`
...
```bash
# S1: static
PRUNE_STRATEGY=S1 PRUNE_RATE=0.5 bash run_pgd.sh

# S2/S3/S4: dynamic, PRUNE_RATE không dùng
PRUNE_STRATEGY=S2 bash run_pgd.sh
PRUNE_STRATEGY=S3 bash run_pgd.sh
PRUNE_STRATEGY=S4 bash run_pgd.sh
```

`run_pgd.sh` sẽ tự map strategy và tạo output root:

| Strategy | Output root |
|---|---|
| `S1`, `PRUNE_RATE=0.5` | `output_static_0.5` |
| `S2` | `output_kneedle` |
| `S3` | `output_otsu` |
| `S4` | `output_gmm` |
...
Nếu không truyền `--output_root`, output mặc định nằm dưới `outputs/`. Nếu chạy qua `run_pgd.sh`, `--output_root` sẽ được set theo strategy, ví dụ `output_static_0.5`, `output_kneedle`, `output_otsu`, hoặc `output_gmm`.
