...
- `proposal branch`: pipeline PGD gồm `teacher -> pruning -> student`, với default teacher `unet_plus_plus` và output chuẩn nằm dưới `outputs/pgd_unet/...`
...
| `S9`, `PRUNE_RATE=0.5`, `STEP3_PRUNING=1`, `STEP3_PRUNING_EPOCHS=4` | `output_full_static_0.5_4` |
| `S10`, `STEP3_PRUNING=1`, `STEP3_PRUNING_EPOCHS=4` | `output_full_kneedle_auto_4` |
| `S11`, `STEP3_PRUNING=1`, `STEP3_PRUNING_EPOCHS=4` | `output_full_otsu_auto_4` |
| `S12`, `STEP3_PRUNING=1`, `STEP3_PRUNING_EPOCHS=4` | `output_full_gmm_auto_4` |
...
3. build `pruned student` từ blueprint: S1-S4 dùng `blueprint_unet_plus_plus` khi teacher là `unet_plus_plus`, S5-S8 dùng `middle_pruned_unet_plus_plus`, S9-S12 dùng `full_pruning_unet_plus_plus`
...
2. build lại đúng student architecture từ blueprint (`blueprint_unet_plus_plus`, `middle_pruned_unet_plus_plus`, `full_pruning_unet_plus_plus`, hoặc fallback `PDGUNet`)
...
Chạy baseline/basic models trên nhiều dataset bằng wrapper ở root project:
...
Các pruning/proposal experiments vẫn chạy bằng các script root hiện có:
...
Chọn pruning strategy bằng biến môi trường:
...
Mỗi run giữ nguyên output cũ và có thể có thêm sidecar:
...
`sample_metrics.csv` dùng để sort các case Dice thấp nhất:
...
Statistics chỉ đọc `outputs/`, không train lại:
...
`--dataset-main` chọn dataset ưu tiên cho main paper. Các dataset còn lại vẫn có bảng/hình trong `statistics/outputs/<dataset_name>/` để dùng cho appendix hoặc phân tích bổ sung.
