...
### Pruning strategy S1/S2/S3/S4/S5

Pipeline hiện hỗ trợ 5 strategy ở step 2:
...
- shell ngoài cùng dùng `PRUNE_STRATEGY=S1/S2/S3/S4/S5`
- Python nội bộ dùng `--prune_method static/kneedle/otsu/gmm/middle_static`
...
Nếu không truyền `--output_root`, output mặc định nằm dưới `outputs/`. Nếu chạy qua `run_pgd.sh`, `--output_root` là root chung, mặc định `outputs`; cấu hình strategy và step 3 nằm ở folder con như `output_static_0.5_4`, `output_static_0.5_no`, `output_kneedle_auto_4`, `output_middle_static_0.5_4`, hoặc `output_gmm_auto_no`.
